...
// en estas tablas). Lo que estas pruebas protegen: que el bootstrap de
// Postgres las cree con los enums correctos y sin ningún statement que se
// pise en un cold start concurrente (mismo patrón que schema-bootstrap.test.js),
// que ninguna de las dos tenga una columna con forma de PII, y que en SQLite
// realmente queden creadas y hereden la MISMA retención que ya rige el
// esquema: se borran solas cuando se borra la persona (ON DELETE CASCADE).
...
test('Postgres: el bootstrap crea match_log y contact_log con sus enums', async () => {
  ...
  // Las dos heredan la retención existente: se van con la persona.
...
test('Postgres: ni match_log ni contact_log declaran una columna con forma de PII', async () => {
  const statements = await bootstrapStatements();
  const schema = statements.find((s) => /CREATE TABLE IF NOT EXISTS match_log/i.test(s));
  ...
  for (const [name, block] of [['match_log', matchLogBlock], ['contact_log', contactLogBlock]]) {
...
test('SQLite: las tablas quedan creadas, sin columnas con forma de PII', async () => {
  ...
  try {
    ...
    try {
      for (const table of ['match_log', 'contact_log']) {
...
test('SQLite: match_log y contact_log se borran solos cuando se borra la persona (misma retención que updates/photos)', async () => {
  ...
  try {
    ...
    // directo, igual que las dos tablas nuevas (que hoy no tienen NINGÚN
    // escritor: PR 3 es solo esquema).
