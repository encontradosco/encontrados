...
async function createApp(adapter, matcher) {
  ...
  app.use((req, res) => {
    ...
    res
      .status(404)
      .send(
        `<!doctype html><html lang="es"><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>No encontrado — Aquí</title><body style="font-family:system-ui;max-width:640px;margin:2rem auto;padding:0 1rem"><h1>Página no encontrada</h1><p>Vuelve al <a href="/">inicio</a> para reportar o buscar a alguien.</p><p style="color:#666;font-size:.85rem">Ruta solicitada: <code>${where}</code></p></body></html>`
      );
  });
